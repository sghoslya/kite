...
# 1. Set the number of cores, for example
cores=8

# 2. Run cyclictest
cyclictest -m -p95 -d0 -a 1-8 -t 8 -h400 -q > histfile.txt 

# 3. Get maximum latency
max=`grep "Max Latencies" histfile.txt | tr " " "\n" | sort -n | tail -1 | sed s/^0*//`

# 4. Grep data lines, remove empty lines and create a common field separator
grep -v -e "^#" -e "^$" histfile.txt | tr " " "\t" >histogram 
